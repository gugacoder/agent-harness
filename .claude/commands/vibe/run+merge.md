# Run completo + merge automatico (com encadeamento)

Executa o loop de desenvolvimento incremental inline e, ao completar todas as features, faz merge automaticamente. Suporta multiplos PRPs em sequencia.
...
Exemplos:
- `/kai:vibe:run+merge M5-seo-analytics-golive`
- `/kai:vibe:run+merge M5-seo-analytics-golive M6-feature-x M7-feature-y`
- `/kai:vibe:run+merge .projeto/PRPs/M5-seo-analytics-golive.md`
...
2. Execute a logica do `/kai:vibe:run {prp}` — loop completo ate todas as features serem `passing` ou `skipped`
...
- Cada PRP e processado sequencialmente (run + merge) antes de iniciar o proximo
- Se apenas 1 argumento, comportamento identico ao simples (run + merge)
